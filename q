...
if [ -z "$1" ]; then
    ...
    echo ""
    echo " If you do not see result in 600 seconds, please restart the Jupyter kernel:"
    ...
    until [ -f $script.o$job_id ]; do
        ...
        if [ $timeout == 600 ]; then
            echo ""
            echo ""
            echo "TimeOut 600 seconds: Job is still queued for execution, check for output file later ($script.o$job_id)"
            echo ""
            break
        fi
    done
